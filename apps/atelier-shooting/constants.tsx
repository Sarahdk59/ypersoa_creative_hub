...
🪢 DRAWSTRING / CORD POLICY (ABSOLUTE — hoodies and zoodies only) : If the garment is a hoodie or zoodie, the drawstrings are PURE COTTON ROUND CORDS, EXACTLY MATCHING the body color of the garment (sage hoodie → sage cords ; navy hoodie → navy cords ; beige hoodie → beige cords). The cord ENDS are RAW : either knotted in a simple knot OR cut clean — nothing else. ABSOLUTELY NO : plastic aglet, metal tip, plastic tip, metallic capsule, plastic bead, decorative tassel, brand label, leather end, transparent end, golden tip, silver tip, embellishment of any kind, white cords on a non-white hoodie. The cord is the most basic possible : just rope, knotted or cut, same color as the sweat. This applies especially in macro shots where the cord ends are visible.`;
...
export const SHOTS_CONFIG = {
  ...
  DETAIL: {
    ...
    promptSuffix: "🪢 ATTENTION CORDONS (MACRO) : Si le sweat visible est un hoodie/zoodie, les cordons sont en COTON ROND PUR, EXACTEMENT MÊME COULEUR que le sweat (sauge → sauge, marine → marine), bouts NUS (noués simples ou coupés net), ABSOLUMENT AUCUN embout plastique, métal, capsule, perle ou ferrure de quelque sorte. Cordons sauge sur sweat sauge, JAMAIS blancs. Gros plan macro extrême (objectif 100mm macro) sur la broderie en fil [THREAD_COLOR] et le grain du tissu [MATERIAL]. Focus ultra-précis sur chaque point de couture (stitching) et sur la typographie brodée qui doit être parfaitement lisible. Lumière douce pour un rendu méga naturel. ATTENTION : la broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF, SANS EFFET 3D NI GONFLÉ.",
