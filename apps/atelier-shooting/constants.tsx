...
export const DECOR_STYLES: { value: DecorStyle; label: string; sublabel: string; icon: string }[] = [
  ...
  { value: 'loft',        label: 'Loft Architectural',        sublabel: 'Béton brut, minimalisme absolu',     icon: 'fa-warehouse' },
...
export const DECOR_DESCRIPTIONS: Record<DecorStyle, { short: string; full: string }> = {
  ...
  loft: {
    short: "Brutalist minimalist concrete loft, raw bare concrete walls and floor, glass roof for soft diffused light. Absolute minimalism — no plants or AT MOST one small green plant in a far corner (entirely optional). No furniture clutter.",
    full:  "Pure brutalist architectural loft / minimalist concrete gallery space. Bare RAW POLISHED CONCRETE walls and concrete floor, optionally a glass roof / verrière flooding the space with soft diffused zenith light. ABSOLUTE MINIMALISM : NO decoration, NO furniture clutter, NO styling, NO plants — OR AT MOST ONE small green plant alone in a far corner (entirely optional, never the focus, never multiple plants). The architecture and the model are the only visual elements. Quiet brutalist French aesthetic — gallery-quiet, museum-cool. The model and the embroidered garment are the only visual subjects, the concrete is pure background atmosphere."
  },
  serre: {
...
🚫 NO PRINTED TEXT POLICY (ABSOLUTE) : Generate the final image WITHOUT any printed text, watermark, brand name, copyright mark, social media handle (@ypersoa, @anything), logo, sign, poster, storefront sign, shop name, label, written word, or visible URL ANYWHERE in the background, on accessories, on the model, or elsewhere in the scene. The ONLY text allowed is the embroidered text on the garment itself (which must match the source PNG exactly). NO frame, NO border, NO caption, NO model release info, NO 'Generated by AI' marking. Pure clean editorial photograph only.

⚠️ SPECIFIC EXCLUSION — REFERENCED BRAND NAMES : The brand names that MAY be mentioned in this prompt as STYLE references (A.P.C., Sézane, Maison Labiche, Émoï-Émoï, Octobre Éditions, AMI Paris, Carine Roitfeld, Vanessa Bruno, La Double J, Frankie Shop, MAC, Champion, Nike, Adidas, etc.) are AESTHETIC GUIDES ONLY — they describe the visual mood and quality bar of the photograph. These brand names must ABSOLUTELY NEVER appear as visible text, signs, storefronts, posters, shop windows, awnings, labels, tags, or logos within the generated image. The visible scene contains ZERO real brand identifications. If a Parisian street is shown, the visible storefronts have NO recognizable shop names. If clothing is shown, NO visible brand logos.
...
🔒 INTERNAL COMPOSITION LOCK (ABSOLUTE) : The motif is ONE single LOCKED composition — reproduce it as a faithful, UNIFORMLY-SCALED copy of the source PNG, as if the whole PNG were resized as one single block. The RELATIVE PROPORTIONS, RELATIVE SIZES and RELATIVE POSITIONS of EVERY element inside the motif (each symbol — heart, star, etc. — and each line of text) MUST be IDENTICAL to the source PNG. Critically : a small symbol stays SMALL relative to the text — if the heart in the PNG is about as wide as one letter of the text, it MUST stay about as wide as one letter in the final embroidery. DO NOT enlarge, shrink, bolden, thicken or re-weight the heart (or any symbol) independently of the text. DO NOT make the heart bigger, more prominent or more dominant than it is in the source. DO NOT change the gap/spacing between the symbol and the text, nor between the lines of text. The heart-to-text size ratio, the line spacing and the overall layout are FIXED by the PNG and must be preserved on EVERY shot of the pack (full body, buste, macro alike). ABSOLUTELY AVOID : an oversized heart floating above tiny text, a heart bigger than a letter when the PNG shows it small, re-proportioned or re-centered elements, a symbol re-scaled to fill the frame. Only the OVERALL badge changes size with camera distance — the INTERNAL proportions NEVER change.
...
📍 EMBROIDERY POSITION ON THE GARMENT (ABSOLUTE — invariant across all shots, including macro) : The embroidered badge is PHYSICALLY POSITIONED on the LEFT CHEST PANEL OF THE WEARER (= viewer's RIGHT when looking at the photo), approximately 8-10 cm BELOW THE COLLARBONE, OFFSET TO THE LEFT of the garment's vertical centerline by about 10-15 cm. The badge is NEVER at the center of the chest, NEVER on the sternum, NEVER aligned with the navel, NEVER over the central zip of a zoodie. This position is INVARIANT regardless of camera framing — even in macro shots where only the embroidery and surrounding fabric are visible, the embroidery sits on the LEFT CHEST PANEL, not on the centered chest area. For zoodies (zip-up), the embroidery is clearly to the LEFT of the central zipper with a 3 cm minimum gap. For pullovers (hoodies/sweats/t-shirts), the embroidery is offset to the LEFT of the imaginary vertical centerline.
...
🚫 CORD ENDS — what to draw : (option A) a small visible OVERHAND KNOT in the same cord material, ~5-8 mm bulge of pure cotton at the very end ; OR (option B) a FRAYED RAW CUT with cotton fibers slightly fanning out, like an unfinished rope ; OR (option C) the cord ends DISAPPEAR — tucked back into the hood, hidden behind the kangaroo pocket, falling outside the frame. The cord ends are pure cotton textile only, soft, organic, exactly the same color and material as the cord itself.
...
export const SHOTS_CONFIG = {
  ...
  DETAIL: {
    ...
    promptSuffix: "⚠️ POSITION BRODERIE (CRITIQUE EN MACRO) : la broderie est PHYSIQUEMENT POSITIONNÉE SUR LE PANNEAU GAUCHE DU PORTEUR (= à droite à l'écran), à 8-10 cm sous la clavicule. JAMAIS au centre du buste. Le cadre macro est ZOOMÉ sur cette zone côté cœur — la broderie remplit 60-80% du cadre, mais elle reste physiquement sur le côté gauche du vêtement, pas au centre. CONTEXT HINT : pour confirmer la position côté cœur, le cadre laisse visible (en bokeh crémeux flou sur le bord) UN INDICE LATÉRAL : à GAUCHE du cadre, une amorce de l'épaule ou du début du bras du modèle ; OU à DROITE du cadre, une amorce de la fermeture éclair centrale (pour zoodie) / du centre du vêtement. ⚠️ CADRAGE : Le cadre EXCLUT la capuche, le col, les cordons et leurs extrémités, le visage. ⚠️ TEXTURE DU TISSU : la matière dans le halo autour de la broderie DOIT correspondre EXACTEMENT à [MATERIAL] — pour hoodie/zoodie/sweat → molleton épais brossé 280g maille tricot dense (NEVER a thin jersey, NEVER a t-shirt fabric) ; pour t-shirt → jersey épais lourd. Gros plan macro extrême (objectif 100mm macro, f/2.8-f/4) sur la broderie en fil [THREAD_COLOR]. Focus ultra-précis sur chaque point de couture, typographie parfaitement lisible. Lumière douce naturelle, bokeh crémeux. ATTENTION : la broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.",
...
export const FULL_PACK_LOFT = {
  PLEIN_PIED: {
    label: "Plein Pied Béton",
    prompt: `Generate an image of: Photographie lifestyle éditoriale d'un modèle ultra-réaliste, imparfait et naturel portant un [PRODUIT] en [COULEUR SWEAT] ([MATERIAL]) avec le motif joint brodé en fil [COULEUR FIL] sur l'[EMPLACEMENT] ([DIMENSION] max). Le modèle est photographié en plein pied, posture naturelle et détendue, appuyé contre un mur en béton brut poli. Décor : espace brutaliste minimaliste, parois en BÉTON BRUT POLI nues, sol en béton, optionnellement une verrière diffusant une lumière zénithale douce. MINIMALISME ABSOLU : aucune plante OU au maximum UNE petite plante verte solitaire dans un coin lointain (entièrement optionnelle, jamais le focus). Aucun meuble, aucun prop, aucun styling. L'architecture est le seul personnage. Lumière diffuse douce. Palette béton clair, écru, neutre. Esthétique galerie d'art française quiet brutalism. Objectif 50mm, profondeur de champ douce. ATTENTION : La broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.\${COPYRIGHT_DISCLAIMER}`
  },
  PLAN_AMERICAIN: {
    label: "Plan Américain Béton",
    prompt: `Generate an image of: Photographie lifestyle éditoriale d'un modèle ultra-réaliste, imparfait et naturel portant un [PRODUIT] en [COULEUR SWEAT] ([MATERIAL]) avec le motif joint brodé en fil [COULEUR FIL] sur l'[EMPLACEMENT] ([DIMENSION] max). Plan américain (mi-cuisse), modèle debout ou légèrement adossé contre un mur en béton brut poli (PAS assis sur un meuble — l'architecture est nue). Décor brutaliste minimaliste : parois en béton brut, sol en béton, optionnellement verrière. MINIMALISME ABSOLU : aucune plante OU au maximum UNE petite plante verte au loin. Aucun meuble, aucun prop. Lumière diffuse douce. Palette béton clair, écru. Esthétique galerie quiet brutalism française. Objectif 85mm, profondeur de champ douce. ATTENTION : La broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.\${COPYRIGHT_DISCLAIMER}`
  },
  BUSTE: {
    label: "Buste Béton",
    prompt: `Generate an image of: Photographie lifestyle éditoriale d'un modèle ultra-réaliste, imparfait et naturel portant un [PRODUIT] en [COULEUR SWEAT] ([MATERIAL]) avec le motif joint brodé en fil [COULEUR FIL] sur l'[EMPLACEMENT] ([DIMENSION] max). Cadrage en buste, le modèle devant un mur en BÉTON BRUT POLI nu, lumière douce diffuse caressant le visage et le vêtement. Décor brutaliste minimaliste : aucune plante en arrière-plan, aucun prop, juste le mur de béton. Palette béton clair, écru, neutre. Esthétique galerie quiet brutalism française. Objectif 85mm, profondeur de champ douce. ATTENTION : La broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.\${COPYRIGHT_DISCLAIMER}`
  },
  RAPPROCHE: {
    label: "Plan Rapproché Béton",
    prompt: `Generate an image of: Photographie lifestyle éditoriale d'un modèle ultra-réaliste, imparfait et naturel portant un [PRODUIT] en [COULEUR SWEAT] ([MATERIAL]) avec le motif joint brodé en fil [COULEUR FIL] sur l'[EMPLACEMENT] ([DIMENSION] max). Plan rapproché sur le torse et la broderie, le modèle peut tenir une tasse en céramique artisanale ÉPURÉE (un seul prop minimaliste optionnel). Décor : mur en béton brut poli en arrière-plan, MINIMALISME ABSOLU, aucune plante au premier plan ni en arrière-plan. Lumière douce diffuse. Palette béton clair, écru. Esthétique galerie quiet brutalism française. Objectif 50mm macro, profondeur de champ douce. ATTENTION : La broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.\${COPYRIGHT_DISCLAIMER}`
  },
  MOUVEMENT: {
    label: "Mouvement Béton",
    prompt: `Generate an image of: Photographie lifestyle éditoriale d'un modèle ultra-réaliste, imparfait et naturel portant un [PRODUIT] en [COULEUR SWEAT] ([MATERIAL]) avec le motif joint brodé en fil [COULEUR FIL] sur l'[EMPLACEMENT] ([DIMENSION] max). Modèle marchant lentement le long d'un grand mur en BÉTON BRUT POLI, sol en béton, léger flou de mouvement, dynamique. Décor brutaliste minimaliste : aucune plante (ou UNE seule au loin floutée), aucun meuble, aucun prop. Optionnellement verrière en arrière-plan diffusant une lumière zénithale douce. Palette béton clair, écru. Esthétique galerie quiet brutalism française. Objectif 50mm. ATTENTION : La broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.\${COPYRIGHT_DISCLAIMER}`
  },
  MACRO_PORTEE: {
    label: "Macro Broderie Béton",
    prompt: `Generate an image of: Hyper-realistic editorial macro photograph showing the embroidered detail WORN by a real human model. ⚠️ ULTRA-TIGHT MACRO FRAMING : embroidery fills 60-80% of frame, frame EXCLUDES the hood, the collar, the shoulders, the drawstrings and their ends. Tight close-up framing on the LEFT CHEST embroidery area of a [PRODUIT] in [COULEUR SWEAT] ([MATERIAL]). The attached motif is embroidered in [COULEUR FIL] thread on the [EMPLACEMENT] ([DIMENSION] max). The shot MUST include a fragment of the model — partial face, hand grazing the embroidery edge, or a strand of hair. THIS IS NOT A FLAT LAY. Background : softly blurred raw polished concrete wall — pure brutalist minimalism, NO plants, NO clutter. Soft diffused natural light. Quiet brutalist French gallery aesthetic. Objectif Canon 100mm macro, f/3.5, ultra-sharp on embroidery with creamy bokeh on concrete. ATTENTION : La broderie doit être EXTRÊMEMENT PLATE, SANS AUCUN RELIEF.\${COPYRIGHT_DISCLAIMER}`
  }
};
