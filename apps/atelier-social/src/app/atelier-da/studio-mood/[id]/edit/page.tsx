...
export default function EditEpisodePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/atelier-da/studio-mood/${id}`);
    } catch (e) {
  ...
  if (error || !ep) return (
    <div style={{ padding: 32 }}>
      <Link href={`/atelier-da/studio-mood/${id}`} style={backLinkStyle}><ArrowLeft size={14} /> Retour</Link>
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      <Link href={`/atelier-da/studio-mood/${id}`} style={backLinkStyle}>
      ...
      <form onSubmit={handleSubmit} style={{ marginTop: 20 }}>
        ...
        <div style={{ marginTop: 28, display: "flex", gap: 10 }}>
          ...
          <Link href={`/atelier-da/studio-mood/${id}`} style={cancelStyle}>Annuler</Link>
        </div>
      </form>
    </div>
  );
