...
/**
 * LibraryPacksPanel — bibliothèque des packs sociaux sauvegardés, en page
 * pleine (onglet "Packs sociaux" de /bibliotheque). Contenu identique à
 * l'ancien LibraryDrawer (modal plein écran ouvert depuis Atelier Social),
 * dépouillé de son chrome de modal (fond, croix de fermeture) — cf. fusion
 * Bibliothèque du 20/08/2026.
 */
...
export function LibraryPacksPanel() {
  ...
  }, [selectedCollection, platformFilter, favoriteOnly]);
    ...
    return (
      <div>
        <button
          ...
          className="mb-4 flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800"
        >
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs mb-4 pb-4 border-b border-slate-100">
        ...
        {(["all", "instagram", "pinterest"] as const).map((p) => (
          <button
            ...
            className={`px-2 py-0.5 rounded ${
              platformFilter === p ? "bg-slate-800 text-white" : "text-slate-500 hover:text-slate-800"
            }`}
          >
      ...
      {!loading && packs.length === 0 && (
        <div className="text-center text-slate-400 text-sm py-12">
          Aucun pack pour l&apos;instant. Génère un pack dans Atelier Social et clique ❤️ &quot;Sauvegarder dans le hub&quot;.
        </div>
      )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {filteredPacks.map((pack) => (
            ...
            >
              ...
              <div className="p-3">
                <div className="text-xs font-bold text-slate-800 truncate">{pack.title || "Sans titre"}</div>
    ...
    <div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          ...
          >
            ...
            {downloading ? "Préparation du ZIP..." : `Télécharger pack (.zip + caption + metadata)`}
          </button>
          <div className="grid grid-cols-2 gap-2">
            {pack.image_urls.map((url, idx) => (
              <div key={idx} className="relative group">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={url} alt={`slide ${idx + 1}`} className="w-full aspect-[4/5] object-cover rounded-lg shadow-sm" />
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold border flex items-center gap-1.5 ${
                pack.is_favorite ? "bg-rose-500 text-white border-rose-500" : "bg-white text-rose-500 border-rose-200"
              }`}
            >
          ...
          {pack.caption_hooks && Object.keys(pack.caption_hooks).length > 0 && (
            <div>
              <label className="text-xs font-semibold text-slate-700 uppercase mb-2 block">Hooks alternatifs</label>
          ...
          <div className="flex gap-2 pt-3 border-t border-slate-100">
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm font-semibold flex items-center gap-1 ${
                confirmDelete ? "bg-red-500 text-white hover:bg-red-600" : "border border-red-200 text-red-500 hover:bg-red-50"
              }`}
            >
      ...
      {catalogTarget && (
        <AddToCatalogModal
          ...
          onSaved={() => { /* no-op : la lib catalog est lue depuis /bibliotheque/motifs */ }}
        />
      )}
    </div>
  );
