  ...
  const handleSelectKey = async () => {
    if (window.aistudio?.openSelectKey) {
      try {
        ...
        console.error("Failed to open API key selection:", err);
      }
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-yp-linen p-8">
        <div className="bg-white p-12 rounded-3xl shadow-2xl max-w-lg text-center border border-yp-sable/20">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-yp-olive mb-4">Clé API Requise</h1>
          <p className="text-gray-600 mb-8 leading-relaxed">
            Pour générer des images en ultra-haute définition avec le modèle Gemini 3.1 Flash Image, vous devez sélectionner votre propre clé API Google Cloud.
            <br/><br/>
            <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="text-yp-olive underline">En savoir plus sur la facturation</a>
          </p>
          <button 
            onClick={handleSelectKey}
            className="bg-yp-olive text-white px-8 py-4 rounded-xl font-medium hover:bg-yp-deepOlive transition-all shadow-md hover:shadow-lg w-full"
          >
